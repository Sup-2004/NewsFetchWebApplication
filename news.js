// news.js
const NEWS_API_KEY = "576f2e330db841ec902261a3a331a37c"; // Replace with your own key
...
searchBtn.addEventListener('click', () => {
  ...
  if (q) searchEverything(q);
});
searchInput.addEventListener('keypress', e => {
  if (e.key === 'Enter') searchEverything(searchInput.value.trim());
});
...
async function fetchTopHeadlines(category='general') {
  showMessage('Loading...');
  try {
    const url = `https://newsapi.org/v2/top-headlines?country=us&category=${category}&pageSize=12&apiKey=${NEWS_API_KEY}`;
...
async function searchEverything(query) {
  ...
  try {
    const url = `https://newsapi.org/v2/everything?q=${query}&pageSize=12&sortBy=publishedAt&language=en&apiKey=${NEWS_API_KEY}`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.status !== 'ok') return showMessage('Error: ' + data.message);
    renderArticles(data.articles);
    ...
    showMessage('Error loading search results.');
  }
}
...
function toggleFavorite(article, btn) {
  ...
  if (exists) {
    const updated = favs.filter(f => f.url !== article.url);
    localStorage.setItem('nf_favs', JSON.stringify(updated));
    btn.textContent = '☆';
  } else {
...
// Personalized news for user interests
